...
import {
  shouldSwitchToAutoMode,
  getModeDecisionReason,
} from "@/app/lib/conversation-mode-analyzer";
import {
  isAutoresponder,
  getAutoresponderReason,
} from "@/app/lib/autoresponder-detector";
import {
  sendAlertNotification,
  shouldSendNotification,
} from "@/app/lib/alert-notifier";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const messageHash = `${conversation.id}-${message}-${Date.now()
      .toString()
      .slice(0, -3)}`; // Same second
        ...
        return NextResponse.json(
          {
            ...
            message:
              "Duplicate webhook call detected - message already processed",
    ...
    // Get recent messages for context (increased to 10 for better context awareness)
          ...
          return NextResponse.json(
            {
              ...
              mode: "manual",
              message:
                "Message received - manual response required (no staff reply yet)",
              reason,
            },
            {
              headers: {
    ...
    if (recentStaffMessage && !justSwitchedToAuto) {
      const minutesSinceStaffMessage =
        (Date.now() - new Date(recentStaffMessage.created_at).getTime()) /
        1000 /
        60;
    ...
    const aiResult = await generateSmartResponse({
      ...
      unifiedAnalysis: {
        // NEW: Pass unified analysis to avoid duplicate classification
        intent: analysis.intent,
        contentType: analysis.contentType,
        sentiment: analysis.sentiment,
        urgency: analysis.urgency,
    ...
    const uniqueResponses = aiResult.responses.filter(
      (response, index, self) => {
        ...
        for (let j = 0; j < index; j++) {
          ...
          if (similarity > 0.7) {
            // Lowered from 0.8 to catch more duplicates
            console.log(
