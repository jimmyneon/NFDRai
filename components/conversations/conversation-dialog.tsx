  ...
  const handleRetryAI = async () => {
    ...
    try {
      const response = await fetch(
        `/api/conversations/${conversation.id}/retry-ai`,
        {
          method: "POST",
        },
      );
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Failed to retry AI",
        variant: "destructive",
      });
    }
          ...
          <DialogTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {conversationStatus === "blocked" ? (
                ...
                <Badge
                  variant={
                    conversationStatus === "auto" ? "default" : "secondary"
                  }
                >
                  {conversationStatus === "auto"
                    ? "🤖 AI Mode"
                    : "👨‍💼 Manual Mode"}
                </Badge>
              )}
          ...
          >
            {sortedMessages.map((message) => (
              ...
              >
                ...
                <div
                  className={`flex-1 ${message.sender === "customer" ? "" : "text-right"}`}
                >
                  <div
                  ...
                  <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground flex-wrap">
                    ...
                    {message.sender === "customer" &&
                      ...
                      message.sentiment_analysis.length > 0 && (
                        <>
                          <span>•</span>
                          ...
                          {!message.sentiment_analysis[0].should_ai_respond && (
                            <Badge variant="secondary" className="text-xs">
                              {message.sentiment_analysis[0].reasoning.includes(
                                "callback",
                              )
                                ? "📞 Callback"
                                : message.sentiment_analysis[0].reasoning.includes(
