-- EMERGENCY FIX: Direct update to quote_acceptance_workflow
-- Remove ALL references to John, staff, handoff
...
3. NEVER escalate - handle it yourself
...
- Escalate when customer has active quote
- Let vague responses go unconfirmed
- Assume anything - always force explicit yes/no
- Mention escalation or passing to anyone
...
SELECT 
  ...
  version,
  CASE 
    WHEN LOWER(prompt_text) LIKE '%john%' THEN 'STILL HAS JOHN'
    WHEN LOWER(prompt_text) LIKE '%staff%' THEN 'STILL HAS STAFF'
    WHEN LOWER(prompt_text) LIKE '%handoff%' THEN 'STILL HAS HANDOFF'
    ELSE 'CLEAN ✅'
  END as status
FROM prompts
WHERE module_name = 'quote_acceptance_workflow';
