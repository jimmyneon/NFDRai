...
async function runTests() {
  ...
  for (const test of testCases) {
    ...
    try {
      ...
      if (test.expected.sentiment && result.sentiment !== test.expected.sentiment) {
        failures.push(`sentiment: expected ${test.expected.sentiment}, got ${result.sentiment}`)
        testPassed = false
      }
      ...
      if (test.expected.shouldAIRespond !== undefined && result.shouldAIRespond !== test.expected.shouldAIRespond) {
        failures.push(`shouldAIRespond: expected ${test.expected.shouldAIRespond}, got ${result.shouldAIRespond}`)
        testPassed = false
      }
      
      if (test.expected.requiresStaffAttention && !result.requiresStaffAttention) {
        failures.push(`requiresStaffAttention: expected true, got false`)
        testPassed = false
      }
      
      if (test.expected.customerName && result.customerName !== test.expected.customerName) {
        failures.push(`customerName: expected ${test.expected.customerName}, got ${result.customerName}`)
        testPassed = false
      }
      ...
      // Show key results
      console.log(`   Results:`)
      console.log(`      Sentiment: ${result.sentiment} (${result.urgency} urgency)`)
      console.log(`      Intent: ${result.intent} (${result.intentConfidence.toFixed(2)} confidence)`)
      console.log(`      Should respond: ${result.shouldAIRespond}`)
      if (result.customerName) {
        console.log(`      Customer name: ${result.customerName} (${result.nameConfidence.toFixed(2)} confidence)`)
      }
  ...
  console.log(`   Savings: $${savings.toFixed(4)} (${((savings / oldCost) * 100).toFixed(0)}% cheaper)`)
  
  console.log('\n⚡ Performance:')
  console.log(`   Regex cases: ${regexCount}/${testCases.length} (${((regexCount / testCases.length) * 100).toFixed(0)}% free)`)
  console.log(`   AI cases: ${aiCount}/${testCases.length} (${((aiCount / testCases.length) * 100).toFixed(0)}% paid)`)
