    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an intent classifier. Respond ONLY with valid JSON, no markdown formatting.'
          },
          {
            role: 'user',
...
function fallbackClassification(
  ...
  const allText = [message, ...history.map(h => h.text)].join(' ').toLowerCase()
  ...
  // Buyback patterns
  if (lowerMessage.match(/sell|buy my|trade.?in|how much.*worth/)) {
    return {
      intent: 'buyback',
      confidence: 0.80,
      deviceType: extractDeviceType(allText),
  ...
  // Warranty patterns
  if (lowerMessage.match(/warranty|still.*not working|repaired.*before|came back/)) {
    return {
    ...
    const classifications = await Promise.all(
      batch.map(msg => 
        classifyIntent({
          ...
          console.error(`[Batch Classifier] Error for message ${msg.id}:`, error)
